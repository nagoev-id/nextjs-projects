-- Этот SQL-запрос пересоздает таблицу users с правильным типом данных для id (UUID)
-- Внимание: этот скрипт удалит существующую таблицу users и все связанные таблицы!
-- Используйте его только если вы понимаете последствия и сделали резервную копию данных

-- Удаляем все таблицы, зависящие от users
...
-- Создаем таблицу users заново с правильным типом id (UUID)
...
-- Удаляем существующий триггер для обновления updated_at, если он существует
